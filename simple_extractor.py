...
##### Algorithm to generate unique RGB color for more than 1 million categories
...
def main():
    ...
    num_classes: int = dataset_settings[args.dataset]['num_classes']
    input_size: List[int] = dataset_settings[args.dataset]['input_size']
    label: List[str] = dataset_settings[args.dataset]['label']

    # GPU info
    gpus: List[int] = [int(i) for i in args.gpu.split(',')]
    assert len(gpus) == 1
    ...
    ##### Backbone ResNet101 - Loading Checkpoint
    ...
    transform = transforms.Compose([
        ...
        ### LIP ?? ATR ?? PASCAL ?? All same mean, std??
    ])
